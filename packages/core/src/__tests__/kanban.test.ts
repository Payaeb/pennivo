...
cards:
  ...
  it("returns null for non-kanban input (no columns)", () => {
    expect(
      parseKanbanMarkdown("just some random text\nwith no structure"),
    ).toBeNull();
  });
...
cards:
  - id: c1
    ...
    const result = parseKanbanMarkdown(input)!;
    expect(result.columns[0].cards[0].description).toBe(
      "Look into new frameworks",
    );
  });
      ...
      columns: [
        {
          id: "todo",
          title: "To Do",
          cards: [{ id: "c1", title: "Task 1", columnId: "todo", order: 0 }],
        },
        {
          id: "done",
          title: "Done",
...
cards:
  ...
  - id: c2
    ...
    for (let i = 0; i < first.columns.length; i++) {
      ...
      expect(second.columns[i].cards).toHaveLength(
        first.columns[i].cards.length,
      );
    }
  });
});
