...
describe('ganttDataToMermaid', () => {
      ...
      sections: [
        {
          title: 'Section 1',
          tasks: [
            { id: 't1', title: 'Task A', startDate: '2026-04-01', duration: '3d' },
          ],
        },
      ],
    };
  ...
  it('output starts with gantt', () => {
    const data: GanttData = {
      title: '',
      dateFormat: 'YYYY-MM-DD',
      sections: [{ title: 'S', tasks: [{ id: 't1', title: 'T', duration: '1d' }] }],
    };
  ...
  it('status flags appear in output', () => {
    const data: GanttData = {
      ...
      sections: [{
        title: 'S',
        tasks: [
          { id: 't1', title: 'Done Task', status: 'done', startDate: '2026-04-01', duration: '3d' },
          { id: 't2', title: 'Active Task', status: 'active', startDate: '2026-04-04', duration: '2d' },
          { id: 't3', title: 'Critical Task', status: 'crit', startDate: '2026-04-06', duration: '1d' },
        ],
      }],
    };
    const output = ganttDataToMermaid(data);
  ...
  it('serializes dependencies with after keyword', () => {
    const data: GanttData = {
      ...
      sections: [{
        title: 'S',
        tasks: [
          { id: 't1', title: 'First', startDate: '2026-04-01', duration: '3d' },
...
describe('Roundtrip: parse → serialize → parse', () => {
  it('parse a gantt string, serialize it, parse again — result matches original parse', () => {
    ...
    expect(second.sections[0].tasks).toHaveLength(first.sections[0].tasks.length);
    expect(second.sections[0].tasks[0].title).toBe(first.sections[0].tasks[0].title);
    expect(second.sections[0].tasks[1].afterDeps).toEqual(first.sections[0].tasks[1].afterDeps);
  });
