    ...
    columns: [
      {
        ...
        title: 'To Do',
        cards: [
          { id: generateCardId(), title: 'First task', columnId: 'todo', order: 0 },
          { id: generateCardId(), title: 'Second task', columnId: 'todo', order: 1 },
        ],
      },
      {
        id: 'in-progress',
        title: 'In Progress',
        cards: [
          { id: generateCardId(), title: 'Current work', columnId: 'in-progress', order: 0 },
        ],
      },
      {
        id: 'done',
...
export function parseKanbanMarkdown(code: string): KanbanData | null {
  ...
  for (const rawLine of lines) {
    ...
    if (isTopLevel && trimmed === 'columns:') {
      ...
      if (currentColumn?.id) {
        data.columns.push({ id: currentColumn.id, title: currentColumn.title || currentColumn.id, cards: [] });
      }
      currentColumn = null;
      continue;
    }
    ...
    if (section === 'cards') {
      ...
      if (currentCard) {
        ...
        if (colMatch) { currentCard.columnId = colMatch[1].trim(); continue; }

        const titleMatch = trimmed.match(/^title:\s*(.+)/);
        if (titleMatch) { currentCard.title = titleMatch[1].trim(); continue; }

        const descMatch = trimmed.match(/^description:\s*(.+)/);
        if (descMatch) { currentCard.description = descMatch[1].trim(); continue; }

        const labelsMatch = trimmed.match(/^labels:\s*(.+)/);
        if (labelsMatch) {
          currentCard.labels = labelsMatch[1].split(',').map(l => l.trim()).filter(Boolean);
          continue;
        }

        const dueMatch = trimmed.match(/^dueDate:\s*(.+)/);
        if (dueMatch) { currentCard.dueDate = dueMatch[1].trim(); continue; }

        const orderMatch = trimmed.match(/^order:\s*(.+)/);
        if (orderMatch) { currentCard.order = parseInt(orderMatch[1], 10); continue; }
      }
    }
  }
...
export function kanbanDataToMarkdown(data: KanbanData): string {
  ...
  for (const col of data.columns) {
    for (let i = 0; i < col.cards.length; i++) {
      ...
      if (card.labels && card.labels.length > 0) lines.push(`    labels: ${card.labels.join(', ')}`);
