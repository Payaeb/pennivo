...
export type ConfigurableAction = Exclude<ToolbarAction, 'focusMode' | 'toggleTheme' | 'sourceMode'>;

/** Category grouping for auto-divider insertion */
export const ACTION_CATEGORY: Record<ConfigurableAction, string> = {
  bold: 'format', italic: 'format', strikethrough: 'format',
...
export function Toolbar({ activeFormats = new Set(), onAction, sourceMode = false, visibleActions, onCustomize }: ToolbarProps) {
  ...
  const allLeftItems: { action: ConfigurableAction; label: string }[] = config.map(a => ({
    action: a,
    ...
    if (lastCategory !== null && cat !== lastCategory) {
      if (currentGroup.length > 0) {
        groupedElements.push(
          <div className="toolbar-group" key={`g${groupIdx}`}>{currentGroup}</div>
        );
        groupedElements.push(<div className="toolbar-divider" key={`d${groupIdx}`} />);
        groupIdx++;
        currentGroup = [];
      }
    }
...
function renderActionButton(
  ...
  btn: (action: ToolbarAction, label: string, children: React.ReactNode, disabled?: boolean) => React.ReactNode,
  disabled: boolean,
): React.ReactNode {
  switch (action) {
    case 'bold':          return btn('bold',          'Bold',          <b>B</b>, disabled);
    case 'italic':        return btn('italic',        'Italic',        <em>I</em>, disabled);
    case 'strikethrough': return btn('strikethrough', 'Strikethrough', <s>S</s>, disabled);
    case 'h1':            return btn('h1',            'Heading 1',     <span className="tool-label-sm">H1</span>, disabled);
    case 'h2':            return btn('h2',            'Heading 2',     <span className="tool-label-sm">H2</span>, disabled);
    case 'bulletList':    return btn('bulletList',     'Bullet List',   <BulletListIcon />, disabled);
    case 'orderedList':   return btn('orderedList',    'Ordered List',  <span className="tool-label-sm">1.</span>, disabled);
    case 'taskList':      return btn('taskList',       'Task List',     <TaskListIcon />, disabled);
    case 'blockquote':    return btn('blockquote',     'Blockquote',    <BlockquoteIcon />, disabled);
    case 'link':          return btn('link',           'Link',          <LinkIcon />, disabled);
    case 'image':         return btn('image',          'Image',         <ImageIcon />, disabled);
    case 'code':          return btn('code',           'Code',          <CodeIcon />, disabled);
    case 'table':         return btn('table',          'Table',         <TableIcon />, disabled);
