...
export function ToolbarCustomizer({
  ...
  const [toolbarItems, setToolbarItems] =
    useState<ConfigurableAction[]>(config);
  const [dragItem, setDragItem] = useState<{
    action: ConfigurableAction;
    source: "toolbar" | "available";
  } | null>(null);
  const [dropTarget, setDropTarget] = useState<{
    index: number;
    list: "toolbar" | "available";
  } | null>(null);
  const panelRef = useRef<HTMLDivElement>(null);

  const availableItems = ALL_CONFIGURABLE_ACTIONS.filter(
    (a) => !toolbarItems.includes(a),
  );

  // Sync changes live
  const commit = useCallback(
    (items: ConfigurableAction[]) => {
    ...
    <div className="toolbar-customizer-backdrop">
      <div className="toolbar-customizer" ref={panelRef}>
        <div className="toolbar-customizer-header">
          ...
          <button
            className="toolbar-customizer-close"
            onClick={onClose}
            aria-label="Close"
          >
            <CloseIcon />
        ...
        <div className="toolbar-customizer-body">
          <div className="toolbar-customizer-column">
            <div className="toolbar-customizer-column-header">
              ...
              <span className="toolbar-customizer-column-count">
            ...
            >
              {availableItems.length === 0 && (
                <div className="toolbar-customizer-empty">
                  All items are in the toolbar
                </div>
              )}
          ...
          <div className="toolbar-customizer-column">
            ...
            >
              ...
              {toolbarItems.map((action, index) => {
                const showDivider =
                  index > 0 &&
                  ACTION_CATEGORY[toolbarItems[index - 1]] !==
                    ACTION_CATEGORY[action];
                return (
                  <div key={action}>
                    {showDivider && (
                      <div className="toolbar-customizer-group-divider" />
                    )}
                    <div
              ...
              {toolbarItems.length > 0 && (
                <div
                  ...
                  onDragOver={(e) =>
                    handleDragOver(e, toolbarItems.length, "toolbar")
                  }
                  onDragLeave={handleDragLeave}
        ...
        <div className="toolbar-customizer-footer">
          <button
            className="toolbar-customizer-reset"
...
function ActionIcon({ action }: { action: ConfigurableAction }) {
  switch (action) {
    case "bold":
      return <b style={{ fontSize: 12, fontWeight: 700 }}>B</b>;
    case "italic":
      return <em style={{ fontSize: 12 }}>I</em>;
    case "strikethrough":
      return <s style={{ fontSize: 12 }}>S</s>;
    case "h1":
      return <span style={{ fontSize: 10, fontWeight: 700 }}>H1</span>;
    case "h2":
      return <span style={{ fontSize: 10, fontWeight: 700 }}>H2</span>;
    case "bulletList":
      return <BulletIcon />;
    case "orderedList":
      return <span style={{ fontSize: 10, fontWeight: 700 }}>1.</span>;
    case "taskList":
      return <TaskIcon />;
    case "blockquote":
      return <QuoteIcon />;
    ...
    case "kanban":
      return <KanbanSmIcon />;
    case "mermaid":
      return <MermaidSmIcon />;
    case "gantt":
      return <GanttSmIcon />;
    case "typewriterMode":
      return <TypewriterSmIcon />;
    default:
      return null;
  }
}
