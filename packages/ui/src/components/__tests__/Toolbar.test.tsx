...
import { Toolbar, DEFAULT_TOOLBAR_CONFIG, TOOLTIP_DATA } from '../Toolbar/Toolbar';
...
describe('Toolbar', () => {
  ...
  describe('Active State', () => {
    ...
    it('multiple buttons can be active simultaneously', () => {
      ...
      expect(screen.getByLabelText('Link')).toHaveAttribute('aria-pressed', 'false');
    });

    it('active state updates when activeFormats prop changes', () => {
      const { rerender } = render(<Toolbar activeFormats={new Set(['bold'])} />);
      expect(screen.getByLabelText('Bold')).toHaveAttribute('aria-pressed', 'true');
  ...
  describe('Source Mode', () => {
    it('formatting buttons are aria-disabled in source mode', () => {
      ...
      expect(screen.getByLabelText('Bold')).toHaveAttribute('aria-disabled', 'true');
      expect(screen.getByLabelText('Italic')).toHaveAttribute('aria-disabled', 'true');
      expect(screen.getByLabelText('Link')).toHaveAttribute('aria-disabled', 'true');
    });
