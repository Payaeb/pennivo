...
export function TableSizePicker({
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (
        ...
        switch (e.key) {
          case "ArrowDown":
            handleCellHover(Math.min(r + 1, 9), c);
            break;
          case "ArrowUp":
            handleCellHover(Math.max(r - 1, 0), c);
            break;
          case "ArrowRight":
            handleCellHover(r, Math.min(c + 1, 9));
            break;
          case "ArrowLeft":
            handleCellHover(r, Math.max(c - 1, 0));
            break;
          case "Enter":
          case " ":
            handleClick(r, c);
  ...
  return (
    ...
    >
      ...
      <div className="table-size-label" aria-live="polite">
        {label}
      </div>
    </div>
  );
