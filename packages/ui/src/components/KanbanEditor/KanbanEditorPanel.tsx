...
export function KanbanEditorPanel({
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(t); document.removeEventListener('mousedown', handleDown); };
  }, [editingCardId]);
  ...
  const addColumn = useCallback(() => {
    ...
    updateData(d => ({
      ...d,
      columns: [...d.columns, { id, title: `Column ${d.columns.length + 1}`, cards: [] }],
    }));
  }, [updateData]);

  const removeColumn = useCallback(
    (colId: string) => updateData(d => ({
      ...d,
  ...
  const addCard = useCallback(
    (colId: string) => {
      ...
      updateColumn(colId, col => ({
        ...col,
        cards: [...col.cards, { id: cardId, title: 'New card', columnId: colId, order: col.cards.length }],
      }));
  ...
  const handleDrop = useCallback((e: React.DragEvent, targetColId: string) => {
    ...
    const cardEls = Array.from(cardsContainer.querySelectorAll('.kanban-card'));
          ...
          <div key={col.id} className="kanban-col">
            <div className="kanban-col-header">
              <input
                ...
                type="text"
                value={col.title}
                onChange={e => updateColumn(col.id, c => ({ ...c, title: e.target.value }))}
            ...
            >
              {col.cards.map(card => (
                ...
                >
                  ...
                  {card.labels && card.labels.length > 0 && editingCardId !== card.id && (
                    <div className="kanban-card-labels">
                      {card.labels.map(l => (
                        <span key={l} className="kanban-card-label">{l}</span>
                      ))}
                    </div>
                  )}
                  ...
                  {editingCardId === card.id && (
                    <div className="kanban-card-edit">
                      ...
                      <input
                        ...
                        autoFocus
                      />
                      <span className="kanban-card-edit-label">Description</span>
                      <input
                        className="kanban-card-edit-input"
                        type="text"
                        value={card.description || ''}
                        onChange={e => updateCard(card.id, c => ({ ...c, description: e.target.value || undefined }))}
                        placeholder="Optional description"
                      />
                      <span className="kanban-card-edit-label">Labels (comma-separated)</span>
                      <LabelsInput
                        labels={card.labels}
                        onChange={labels => updateCard(card.id, c => ({ ...c, labels: labels.length > 0 ? labels : undefined }))}
                      />
                      ...
                      <input
                        ...
                        type="date"
                        value={card.dueDate || ''}
                        onChange={e => updateCard(card.id, c => ({ ...c, dueDate: e.target.value || undefined }))}
                      />
                    </div>
                  )}
                </div>
...
function LabelsInput({ labels, onChange }: { labels?: string[]; onChange: (labels: string[]) => void }) {
  ...
  const commit = useCallback(() => {
    const parsed = text.split(',').map(l => l.trim()).filter(Boolean);
    onChange(parsed);
  }, [text, onChange]);
